...
import { get, remove, set, globalContext, KEY } from '@analytics/global-storage-utils';
import { isObject, PREFIX, isFunction, isBoolean, isString, isBrowser, isArray as isArray$1 } from '@analytics/type-utils';
...
function ensureArray(singleOrArray) {
  ...
  if (isArray$1(singleOrArray)) return singleOrArray;
...
function analytics() {
  ...
  if (isBrowser && config.debug) {
    ...
    composeWithGlobalDebug = function composeWithGlobalDebug() {
      ...
      if (isObject(_typeof(arguments[0]))) return composeWithDebug();
...
export { analytics as Analytics, constants as CONSTANTS, EVENTS$1 as EVENTS, analytics as default, analytics as init };
